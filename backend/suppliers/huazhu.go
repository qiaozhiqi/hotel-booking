...
func (h *HuazhuAdapter) GetPriority() int {
	return 1
}

func (h *HuazhuAdapter) GetColor() string {
	return "#FF6B35"
}

func (h *HuazhuAdapter) GetIcon() string {
	return "🏨"
}
...
func generateHuazhuRooms(hotelIndex int, basePrice int) []SupplierRoomData {
	roomTypes := []struct {
		...
		{"标准双床房", "经济实惠的标准客房，两张单人床，适合商务出行。配备免费WiFi、空调、电视和24小时热水。", 1.0, 2, 28, "双床", "标准双床房"},
		{"标准大床房", "舒适的标准客房，一张大床，适合情侣或单人入住。配备免费WiFi、空调、电视和24小时热水。", 1.1, 2, 30, "大床", "标准大床房"},
		{"豪华大床房", "宽敞明亮的豪华客房，配备高品质床品和现代化设施。享有城市美景，是商务和休闲的理想选择。", 1.5, 2, 40, "大床", "豪华大床房"},
		{"行政套房", "高端行政套房，独立客厅和卧室，配备宽大办公桌和高速网络。享受行政楼层礼遇，适合高端商务人士。", 2.5, 2, 65, "大床", "行政套房"},
		{"家庭房", "温馨舒适的家庭房，配备大床和单人床，适合家庭出行。空间宽敞，设施齐全，让您享受家的温暖。", 1.8, 4, 50, "大床+双床", "家庭房"},
	}
	
	promotionTags := []string{"限时特惠", "会员专享", "提前预订优惠", "连住优惠", "", "", ""}
